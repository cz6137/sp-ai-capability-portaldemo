-- Upgrade an existing SP AI Portal database for the 0818 meeting requirements.
-- Execute as the application schema owner. New installations should use Flyway.
BEGIN;
...
BEGIN
  IF NOT EXISTS (
    ...
    WHERE conname = 'uk_skill_version_name'
      AND conrelid = 'sp_ai_portal.skill_version'::regclass
  ) THEN
...
ALTER TABLE asset ADD COLUMN IF NOT EXISTS case_category_id varchar(36) REFERENCES case_category(id);
CREATE INDEX IF NOT EXISTS idx_asset_case_category ON asset(case_category_id, enabled, status);
CREATE INDEX IF NOT EXISTS idx_skill_status_download ON skill(status, enabled, download_count DESC);
